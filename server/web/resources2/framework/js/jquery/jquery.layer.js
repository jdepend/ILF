...
pathUrl = '../../css/html/', //当前js所在路径，上述变量为false才有效
...
Class.pt.space = function(html){
	//记住选择
	var rember="<br/><input type=\"checkbox\" id=\"qeweb_display_status\" /> 是否以后不弹出此类提示框？";
	
	var html = html || '', times = this.index, config = this.config, dialog = config.dialog, frame = [
		'<div class="xubox_dialog"><span class="xubox_msg xulayer_png32 xubox_msgico xubox_msgtype' + dialog.type + '"></span><span class="xubox_msg xubox_text">' + dialog.msg + (config.remember?rember:'') +'</span></div>',	
...
Class.pt.creat = function(){
	...
	//注释 Class.pt.type = ['dialog', 'page', 'iframe', 'loading', 'tips'];
	switch(config.type){
		case 1:
			if(page.html !== ''){
