...
function ButtonEvent() {
	...
	this.buttonDom;				//按钮的dom, 即button控件
	...
	this.init = function(operateDom) {
		...
		if(operateDom.boDom) {
			this.boDom = operateDom.boDom;
		}
		else if(operateDom.attr(DISLAND.OPERATE_IDX)) {
	...
	/**
	 * 设置containerDom, 当数据岛和Vc组件不能一一对应时使用
	 * (table的增/改/查看弹出框公用同一个数据岛, 此时同一个数据岛对应三个formPanel)
	 */
	...
	this.getDataStr = function() {
		//按钮所在粗粒度组件的boId
		var sourceName = "";
		sourceName = this.containerDom ? this.containerDom.name : this.boDom.attr(DISLAND.BIND);
