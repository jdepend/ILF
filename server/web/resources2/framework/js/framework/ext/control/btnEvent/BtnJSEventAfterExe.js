/**
 * 按钮的JS事件处理者,在执行bo的方法后,页面流跳转前执行  extends ButtonEvent
 */
function BtnJSEventAfterExe() {
	BtnJSEventAfterExe.superclass.constructor.call(this);
	...
	this.handleRequest = function(operateDom) {
		...
		var jsMethods = operateDom.attr(DISLAND.OPERATE_JS_AFTER);
		//JS函数的执行结果
		var result = true;
...
extend(BtnJSEventAfterExe, ButtonEvent);

/**
 * 将数据同步到数据岛
 */
BtnJSEventAfterExe.prototype.bindData = function() {
	//表单中的按钮
