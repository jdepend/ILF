 ...
 * BtnFormExportEvent的handleRequest负责处理表单的导出方法
 *
 */
...
BtnFormExportEvent.prototype.execute = function(operateDom) {
	this.bindData();
	var sourceName = this.containerDom ? this.containerDom.name : this.boDom.attr(DISLAND.BIND);
	var	url = appConfig.ctx + actionURL.doExport()
	...
	var paramVal = xmlToString(getDataIslandByJQ());
	if(formA) {
