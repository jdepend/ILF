...
Ext.extend(Ext.ux.form.TimePickerField, Ext.form.Field, {
    ...
    onRender: function(ct, position){
        ...
        this.minutesSpinner = new Ext.ux.form.SpinnerField(Ext.apply({}, cfg, {
            ...
            maxValue: 59,
            allowDecimals : false,
            value: this.ownerCt.format == 'Y-m-d H' ? '0' : values.m,
            disabled : this.ownerCt.format == 'Y-m-d H'
        }));
        this.secondsSpinner = new Ext.ux.form.SpinnerField(Ext.apply({}, cfg, {
            minValue: 0,
            maxValue: 59,
            allowDecimals : false,
            value: (this.ownerCt.format == 'Y-m-d H:i' || this.ownerCt.format == 'Y-m-d H') ? '0' : values.s,
            disabled : this.ownerCt.format == 'Y-m-d H:i' || this.ownerCt.format == 'Y-m-d H'
        }));
...
Ext.DateTimePicker = Ext.extend(Ext.DatePicker, {
    timeFormat: 'g:i:s A',
    timeLabel: '<font style="font-size:12px;">' + I18N.DATE_TIMELABEL + '</font>',
...
Ext.ux.form.DateTimeField = Ext.extend(Ext.form.DateField, Ext.form.TwinTriggerField,{
	/**beg**/
...
        this.fireEvent("clear", this);
		if(this.getId().split(DISLAND.SPLIT_EXPEND).length>1 && this.getId().split(DISLAND.SPLIT_EXPEND)[1] == DISLAND.BOP_RANGE_MIN){
			Ext.getCmp(this.getId().split(DISLAND.SPLIT_EXPEND)[0] + DISLAND.SPLIT_EXPEND + DISLAND.BOP_RANGE_MAX).setMinValue(this.getValue() || '1900-1-1');
		}
		else if(this.getId().split(DISLAND.SPLIT_EXPEND).length>1 && this.getId().split(DISLAND.SPLIT_EXPEND)[1] == DISLAND.BOP_RANGE_MAX){
			Ext.getCmp(this.getId().split(DISLAND.SPLIT_EXPEND)[0] + DISLAND.SPLIT_EXPEND + DISLAND.BOP_RANGE_MIN).setMaxValue(this.getValue() || '2999-12-31');
		}
    },
...
    onTrigger2Click: function() {
        ...
        if (this.menu == null) {
        	if(this.format == 'Y-m-d' || this.format == 'Y' || this.format == 'Y-m')
            	this.menu = new Ext.menu.DateMenu();
            else
            	this.menu = new Ext.menu.DateTimeMenu();
        }
        else if(this.menu && this.menu.isVisible()){
...
        this.menu.picker.setValue(this.getValue() || new Date());
        ...
        if(this.format != 'Y' && this.format != 'Y-m')
        	return;
        ...
        if(this.format == 'Y' || this.format == 'Y-m') {
			...
			if (!p.disabled) {
				...
				this.getValue = this.getValue.createInterceptor(setDefaultMonthDay).createSequence(restoreDefaultMonthDay);
				this.beforeBlur = this.beforeBlur.createInterceptor(setDefaultMonthDay).createSequence(restoreDefaultMonthDay);
			}
		}
    },
...
Ext.reg('datefield', Ext.ux.form.DateTimeField);
