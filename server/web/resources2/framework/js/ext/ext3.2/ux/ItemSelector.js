...
Ext.ux.form.ItemSelector = Ext.extend(Ext.form.Field,  {
    ...
    setValue: function(val) {
        ...
        var formMultiSel = this.fromMultiselect;
        var toMultiSel = this.toMultiselect;
        if(toMultiSel != undefined && toMultiSel.store != null){
        	toMultiSel.store.removeAll();
        } 
        if(formMultiSel == null || formMultiSel == undefined || toMultiSel == null || toMultiSel == undefined){
        	selectValue = val;
        }else{
	        ...
	        for(i = 0; i < val.length; i++) {
	            ...
	            var records = [];
	            var ind = 0;
	            formMultiSel.store.data.each(function(){
	            	if(this.data.text == text){
	            		var record = formMultiSel.view.store.getAt(ind);	              
						records.push(record);		
						//移除左侧记录
						formMultiSel.view.store.remove(record);
	            	}
	            	ind++;
	            });
	            toMultiSel.view.store.add(records);
	        }
        }
	},
    reset: function(){
