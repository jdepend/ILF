    ...
    onRender: function(){
        this.btn = this.cmp.getToolbar().addButton({
            ...
            handler: function(){
                ...
                this.farWindow.show(Ext.getBody());
...
Ext.ux.form.HtmlEditor.HR = Ext.extend(Ext.util.Observable, {
    // HR language text
    langTitle   : '<span style="font-weight:normal">' + I18N.EDITOR_HR + '</span>',
    langHelp    : I18N.EDITOR_HR,
    langInsert  : '',
    langCancel  : 'Cancel',
    langWidth   : 'Width',
    // defaults
...
Ext.ux.form.HtmlEditor.Image = Ext.extend(Ext.util.Observable, {
	// Image language text
	langTitle : '<span style="font-weight:normal">' + I18N.EDITOR_IMAGE + '</span>',
	...
	selectImage : function() {
		if (!this.imgWindow) {
					...
					items : [{
						...
						buttons : [{
							...
							handler : function() {
								...
								if (frm.isValid()) {
									file1 = frm.findField('attachmentFileName').getValue();
									...
									if (type == ".jpg" || type == ".gif"
										...
										showMSG.showWaring('只支持上传jpg和gif格式的图片文件');
										return;
									}
									frm.submit({
												url : appConfig.ctx + actionURL.uploadImg(),
												waitTitle : I18N.CONFIRM_TIPS,
												method : 'POST',
												waitMsg : I18N.FILE_UPLOADING,
												timeout : 60000, // 60s
												success : function(form, action) {
													if(action.result.success) {
														...
														showMSG.showErr(action.result.msg)
													}
												},
			...
			this.imgWindow.show(Ext.getBody());
		} else {
			this.imgWindow.show(Ext.getBody());
		}
	},
	insertImage : function(img) {
		this.cmp.insertAtCursor('<img src="' + img.Url + '" title="' + img.Title
				+ '" alt="' + img.Title + '">');
	}
...
Ext.ux.form.HtmlEditor.Link = Ext.extend(Ext.util.Observable, {
	// Link language text
	langTitle : '<span style="font-weight:normal">' + I18N.EDITOR_LINK + '</span>',
	langInsert : '插入',
	langCancel : '取消',
	langTarget : '打开目标',
	langURL : '链接地址',
						...
						buttons : [{
							...
							handler : function() {
								...
								if (frm.isValid()) {
									url = frm.findField('url').getValue();
									if (!this.isUrl(url)) {
										Ext.Msg.alert('提示', 'URL不合法.请重新输入.格式[http://www.****]');
										return;
									};
									var afterSpace = '', 
...
//									}
					...
					this.linkWindow.show(Ext.getBody());
				} else {
					this.linkWindow.show(Ext.getBody());
...
/**
 * @class Ext.ux.form.HtmlEditor.Clear
 * @extends Ext.ux.form.HtmlEditor.MidasCommand
 * <p>A plugin that creates a button on the HtmlEditor that will clear text.</p>
 */
Ext.ux.form.HtmlEditor.Clear = Ext.extend(Ext.util.Observable, {
    // HR language text
    langTitle   : '<span style="font-weight:normal">' + I18N.EDITOR_CLEAR + '</span>',
    langHelp    : I18N.EDITOR_CLEAR,
    langCancel  : 'Cancel',
    langWidth   : 'Width',
    // defaults
    defaultHRWidth: '100%',
    ...
    onRender: function(){
        ...
        var btn = this.cmp.getToolbar().addButton({
            ...
            handler: function(){
                ...
                this.charWindow.show(Ext.getBody());
            },
            scope: this,
            tooltip: {
                title: this.langTitle
...
Ext.ux.form.HtmlEditor.Table = Ext.extend(Ext.util.Observable, {
    // Table language text
    langTitle       : '<span style="font-weight:normal">' + I18N.EDITOR_TABLE + '</span>',
    langInsert      : '插入',
    langCancel      : '取消',
    langRows        : '行数',
    langColumns     : '列数',
    ...
    onRender: function(){
        var btn = this.cmp.getToolbar().addButton({
            ...
            handler: function(){
                ...
                this.tableWindow.show(Ext.getBody());
            },
            scope: this,
            tooltip: {
                title: this.langTitle
...
Ext.ux.form.HtmlEditor.Word = Ext.extend(Ext.util.Observable, {
    // Word language text
    langTitle: '<span style="font-weight:normal">' + I18N.EDITOR_WORD + '</span>',
    langToolTip: '<span style="font-weight:normal">' + I18N.EDITOR_WORD + '</span>',
...
Ext.ux.form.HtmlEditor.plugins = function(){
    return [
        ...
        new Ext.ux.form.HtmlEditor.Clear()
    ];
};