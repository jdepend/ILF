...
public class SrcHandle extends PackageHandle{

	@Override
	public void modifyPackage(ProjectModuleBO bo, String oldPath) throws BOException {
		String rootPath = ProjectPathUtil.getProjectPath()+ ImpConfigConstant.DEFAULT_SRCPATH;
		String path = rootPath + (bo.getSrcPackage().replaceAll("\\.", "/"));
		oldPath = rootPath + (oldPath.replaceAll("\\.", "/"));
	...
	protected String getPath(ProjectModuleBO bo) {
		return ProjectPathUtil.getProjectPath() + ImpConfigConstant.DEFAULT_SRCPATH 
			+ (bo.getSrcPackage().replaceAll("\\.", "/"));
	}
