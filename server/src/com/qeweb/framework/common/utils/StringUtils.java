...
public class StringUtils {
	...
	final public static String encode(String str) {
		try {
			return isNotEmpty(str) ? URLEncoder.encode(str, Constant.ENCODING_UTF8) : "";
	...
	final public static String decode(String str) {
		try {
			return isNotEmpty(str) ? URLDecoder.decode(str, Constant.ENCODING_UTF8) : "";
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
			return "";
		}
	...
	/**
	 * 将Collection转换为String,中间用split分隔
	 * @param list
	 * @param split
	 * @return
	 */
	final public static String revertColl2Str(Collection<String> cols, String split) {
		if(ContainerUtil.isNull(cols))
			return "";

		StringBuilder sbr = new StringBuilder();
		for(String s : cols) {
			if(isNotEmpty(s))
				sbr.append(s).append(split);
		}

		return MatcherUtil.removeEnd(sbr, split);
	}
	...
	public static String toString(Object obj, int scale) {
		String formatStr = "%." + (scale > 0 ? scale : 8) + "f";
		String value = "";
		if(obj == null)
			return value;
		if(obj.getClass() == double.class
			...
			if(scale != -1)
				value = String.format(formatStr, obj);
			else
				value = obj + "";
		}
			...
			value = DateUtils.dateToString((Date) obj, DateUtils.DATE_FORMAT_YYYY_MM_DD_HH_MM_SS);
		}
